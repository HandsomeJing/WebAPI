...
{
    /// <summary>
    /// 数据库上下文 - Entity Framework Core的核心类
    /// 负责管理数据库连接、实体映射、数据操作等
    /// </summary>
    public partial class DearlerPlatformDbContext : DbContext
    {
        public DearlerPlatformDbContext()
        {
        }

        public DearlerPlatformDbContext(DbContextOptions<DearlerPlatformDbContext> options)
            : base(options)
        {
        }
        ...
        {
            // 数据库连接配置已移到appsettings.json，这里保留作为备用
            if (!optionsBuilder.IsConfigured)
            {
                // 注意：生产环境应该从配置文件读取连接字符串
                optionsBuilder.UseSqlServer("server=127.0.0.1,1400;uid=sa;pwd=Jst1996520@;database=DearlerPlatformDb;TrustServerCertificate=true;");
            }
        }
